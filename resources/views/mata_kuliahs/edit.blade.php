...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Mata Kuliah</h2>
        <p class="text-slate-500 dark:text-slate-400 text-sm">Perbarui informasi mata kuliah</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8 transition-colors">
        <form action="{{ route('mata-kuliah.update', $mataKuliah) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $mataKuliah->nama) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 dark:focus:ring-indigo-900 outline-none transition-all placeholder-slate-400 dark:placeholder-slate-500" required>
            </div>
