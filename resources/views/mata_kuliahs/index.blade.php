    ...
    <div x-show="open" 
        ...
        <div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
            <div x-show="open" 
                ...
                <form :action="isEditing ? '{{ url('mata-kuliah') }}/' + form.id : '{{ route('mata-kuliah.store') }}'" method="POST" class="p-6">
                    @csrf
                    ...
                    <div class="mb-4">
                        <h3 class="text-lg font-medium leading-6 text-gray-900" x-text="isEditing ? 'Edit Mata Kuliah' : 'Tambah Mata Kuliah'"></h3>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <!-- Kode -->
                            <div>
                                <label for="kode" class="block text-sm font-medium text-slate-700">Kode MK</label>
                                <input type="text" name="kode" x-model="form.kode" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-all font-mono" required placeholder="Contoh: IF101">
                            </div>
                        ...
                        <!-- Nama -->
                        <div>
                            <label for="nama" class="block text-sm font-medium text-slate-700">Nama Mata Kuliah</label>
                            <input type="text" name="nama" x-model="form.nama" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-all" required placeholder="Contoh: Algoritma dan Pemrograman">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                             <!-- SKS -->
                            <div>
                                <label for="sks" class="block text-sm font-medium text-slate-700">Jumlah SKS</label>
                                <input type="number" name="sks" x-model="form.sks" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-all" required min="1" max="6">
                            </div>

                            <!-- Semester -->
                            <div>
                                <label for="semester" class="block text-sm font-medium text-slate-700">Semester</label>
                                <input type="number" name="semester" x-model="form.semester" class="mt-1 w-full px-3 py-2 rounded-lg border border-slate-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-all" required min="1" max="8">
                            </div>
                        </div>
                    </div>
